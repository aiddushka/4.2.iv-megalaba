...
def main() -> None:
    parser = argparse.ArgumentParser(description="MQTT irrigation actuator emulator")
    ...
    def on_message(_client: mqtt.Client, _userdata: object, msg: mqtt.MQTTMessage) -> None:
        ...
        if abs(now_ts - ts) > allowed_drift:
            return
        if str(nonce) in used_nonces:
            return
        used_nonces.add(str(nonce))

        base_str = command_base_str(device_uid, ACTUATOR_TYPE, str(action), ts, str(nonce))
